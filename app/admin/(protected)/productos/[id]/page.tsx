...
type VariantOption = { id?: string; name: string; order: number }
type VariantGroup = { id?: string; name: string; required: boolean; order: number; options: VariantOption[] }
...
export default function EditProductoPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setShowStock(product.showStock ?? false)
      ...
      if (product.badge) {
        ...
        setBadgeColor(product.badge.color ?? 'red')
        setBadgeType(product.badge.type ?? 'custom')
      }
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          ...
          className="flex items-center gap-2 bg-red-500 text-white text-sm px-4 py-2 rounded-lg hover:bg-red-600 transition-colors"
        >
          <Trash2 size={14} />
          Eliminar
        </button>
          ...
          <div className="lg:col-span-2 space-y-5">
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-4">
              ...
              <div>
                ...
                <input className={inputCls} value={name} onChange={e => { setName(e.target.value); setSlug(slugify(e.target.value)) }} required />
              </div>
              <div>
              ...
              <div>
                ...
                <textarea className={inputCls + ' resize-y min-h-[100px]'} value={description} onChange={e => setDescription(e.target.value)} />
              </div>
