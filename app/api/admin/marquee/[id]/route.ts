...
export async function PUT(
  ...
  const existing = await db.marqueeText.findFirst({ where: { id, tenantId: tenant.id } })
  if (!existing) return NextResponse.json({ error: 'Not found' }, { status: 404 })

  const { text, order, enabled } = await request.json()

  const marquee = await db.marqueeText.update({
    where: { id },
    data: { text, order, enabled },
  })
...
export async function DELETE(
  ...
  await db.marqueeText.delete({ where: { id } })
  revalidateTag(`tenant-${tenant.slug}`, 'default')
