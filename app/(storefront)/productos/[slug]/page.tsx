...
import { useCartStore, type VariantSelection } from '@/lib/cart'
...
type VariantGroup = { id: string; name: string; required: boolean; options: VariantOption[] }
...
export default function ProductDetailPage() {
  ...
  if (notFound || !product) {
    return (
      <main className="pt-6 min-h-screen flex flex-col items-center justify-center gap-4 text-gray-400">
  ...
  function handleAddToCart() {
    ...
    if (!allRequiredSelected) {
      const missing = requiredGroups.find(g => !selections[g.id])
      toast.error(`Seleccioná ${missing?.name}`)
      return
    }
    const variantSelections: VariantSelection[] = product!.variantGroups
      .filter(g => selections[g.id])
      .map(g => ({
        groupId: g.id,
        groupName: g.name,
        optionId: selections[g.id].id,
        optionName: selections[g.id].name,
      }))
    ...
    const variantLabel = variantSelections.length > 0
      ? ` (${variantSelections.map(v => v.optionName).join(', ')})`
      : ''
    toast.success(`${product!.name}${variantLabel} agregado al carrito`, { duration: 2000 })
  }
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="grid md:grid-cols-2 gap-10">
          ...
          <div className="space-y-5">
            ...
            {/* Stock */}
            {product.showStock && product.trackStock && (
                ...
                {product.variantGroups.map(group => (
                  <div key={group.id}>
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      {group.required && !selections[group.id] && (
                        <span className="text-xs text-gray-400">— Elegí uno</span>
                      )}
                      {selections[group.id] && (
                        <span className="text-xs font-medium text-gray-500">— {selections[group.id].name}</span>
                      )}
                    </div>
                    <div className="flex flex-wrap gap-2">
                      {group.options.map(opt => {
                        const selected = selections[group.id]?.id === opt.id
                        return (
                          <button
                            key={opt.id}
                            type="button"
              ...
              <div className="space-y-3">
                ...
                {!allRequiredSelected && requiredGroups.length > 0 && (
                  <p className="text-xs text-gray-400 text-center">
                    Seleccioná {requiredGroups.filter(g => !selections[g.id]).map(g => g.name).join(' y ')} para continuar
                  </p>
                )}
              </div>
            ) : (
