...
import { MapPin, Mail, Building2, Store, Phone, Clock, Globe, Landmark, Navigation, Warehouse, Coffee, ShoppingBag, Home } from 'lucide-react'

const BRANCH_ICONS: Record<string, React.ElementType> = {
  MapPin, Building2, Store, Home, Landmark, Warehouse, Coffee, ShoppingBag, Phone, Clock, Globe, Navigation,
}
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      <div className="max-w-2xl mx-auto px-4 py-16">
        ...
        <p className="text-gray-500 mb-10">Estamos para ayudarte. Escribinos por cualquiera de estos medios.</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-8">
            {tenant.socialLinks.map((link) => (
              <a
                ...
                className="flex items-center gap-4 bg-white rounded-2xl border p-4 hover:shadow-md transition-shadow group"
              >
                <div
                  className="w-11 h-11 rounded-xl flex items-center justify-center text-white shrink-0"
                  style={{ backgroundColor: PLATFORM_COLORS[link.platform] ?? '#6b7280' }}
                >
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400 truncate max-w-[160px]">{link.url.replace(/^https?:\/\/(www\.)?/, '')}</p>
                </div>
              </a>
            ))}
          </div>
        ...
        {tenant.branches.length > 0 && (
          <div className="mb-8">
            <h2 className="font-bold text-base mb-4">Nuestras sucursales</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {tenant.branches.map(b => {
                const Icon = BRANCH_ICONS[b.icon] ?? MapPin
                return (
                  <div key={b.id} className="bg-white rounded-2xl border p-5 flex gap-4">
                    <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" style={{ backgroundColor: 'var(--color-primary)', opacity: 0.9 }}>
                    ...
                    <div>
                      ...
                      {b.address && (
                        b.mapUrl
                          ? <a href={b.mapUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-gray-500 hover:underline mt-0.5 block">{b.address}{b.city ? `, ${b.city}` : ''} ↗</a>
                      ...
                      {b.mapUrl && !b.address && (
                        <a href={b.mapUrl} target="_blank" rel="noopener noreferrer" className="text-xs mt-1 inline-block hover:underline" style={{ color: 'var(--color-primary)' }}>Ver en mapa ↗</a>
                      )}
                    </div>
                  </div>
                )
        ...
        {/* Contact numbers */}
        {tenant.contactNumbers.length > 0 && (
          <div className="mb-8">
            <h2 className="font-bold text-base mb-4">Teléfonos de contacto</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {tenant.contactNumbers.map(c => (
                <div key={c.id} className="bg-white rounded-2xl border p-5">
                  <p className="font-semibold text-sm mb-2">{c.title}</p>
                  <a href={`https://wa.me/${c.phone.replace(/\D/g, '')}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm hover:underline" style={{ color: 'var(--color-primary)' }}>
                    <Phone size={14} /> {c.phone}
                  </a>
                  {c.email && (
                    <a href={`mailto:${c.email}`} className="flex items-center gap-2 text-sm text-gray-500 hover:underline mt-1">
        ...
        <div className="bg-white rounded-2xl border p-6 space-y-4">
          <h2 className="font-bold text-base">Información directa</h2>
